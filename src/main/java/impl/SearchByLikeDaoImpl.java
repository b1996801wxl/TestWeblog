...
public class SearchByLikeDaoImpl extends BaseDao<BlogContentQuery> implements SearchByLikeDao {
    ...
    public List<BlogContentQuery> SearchByLike(BigDecimal pagenum, BigDecimal num) {
        ...
        return executeQuery("SELECT * FROM(SELECT * FROM(select @rownum:=@rownum+1 AS A,B.*,C.USER_NAME,C.HEAD_IMG from(SELECT COUNT(*) AS NUM1,D.BLOG_ID FROM\n" +
                ...
                "AND B.USER_ID = C.USER_ID)E WHERE A<=?)F WHERE A >=?",new Object[]{pagenum.multiply(num),(pagenum.subtract(new BigDecimal(1))).multiply(num)});
    }
    ...
    public List<BlogContentQuery> NavSearchBlog(String str, BigDecimal pagenum, BigDecimal num) {
        return executeQuery("select* from(SELECT * FROM(SELECT @rownum:=@rownum+1 AS A,C.*,B.USER_NAME,B.HEAD_IMG FROM (\n" +
                "                select * from BLOG_CONTENT  where BLOG_TEXT like ? ) C,USER_INFO B \n" +
                "                WHERE C.USER_ID = B.USER_ID order by BLOG_ID desc)E WHERE A<=?)F where A >=?",
